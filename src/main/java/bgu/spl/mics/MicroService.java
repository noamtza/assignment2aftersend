...
public abstract class MicroService implements Runnable {
    protected ConcurrentHashMap<Class<? extends Message>,Callback> mescall;
//hash map <message, callback>
    private boolean terminated = false;
    private final String name;
    //private Queue<Message> messagesQueue;//want to initialixe at the construcror as null, and "open" it at register-messageBus
    protected MessageBusImpl mesB=MessageBusImpl.getInstance();
    boolean isRegister;
   protected int countTime;
    ...
    public MicroService(String name) {
        this.name = name;
//        messagesQueue=null;
       // mesB=mesB.getInstance();
        isRegister=false;
        mescall=new ConcurrentHashMap<>();
    }
    //we add
    ...
    public void setIsRegister(boolean reg){
        isRegister=reg;
    }
//    public void intializeMessageQ(){
...
//    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        //TODO: implement this.
        mesB.subscribeEvent(type,this);
        mescall.put(type,callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        //TODO: implement this.
        mesB.subscribeBroadcast(type,this);
        mescall.put(type,callback);
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
        //TODO: implement this.
        mesB.sendBroadcast(b);
        if (b instanceof TerminateBroadcast) {
            terminate();
            mesB.unregister(this);
        }
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            try {
                Message m= mesB.awaitMessage(this);
                if(m!=null) {
                    if (mescall.containsKey(m.getClass())) {
                        if (mescall.get(m.getClass()) != null) {
                            mescall.get(m.getClass()).call(m);
                        }
                    }
                }
